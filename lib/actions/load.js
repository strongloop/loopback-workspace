...
class WorkspaceActions {
  ...
  loadModelConfig(filePath, cb) {
    ...
    fsUtility.readFile(filePath, function(err, config) {
      ...
      delete config._meta;
      facet.setModelConfig(config, function(collection, key, data) {
        let modelConfig = new ModelConfig(workspace, key, data);
        collection.add(modelConfig);
      });
      cb();
    });
  }
  loadMiddleware(filePath, cb) {
    ...
    fsUtility.readFile(filePath, function(err, fileData) {
      ...
      names.pop();
      let middleware = facet.middlewares(names.join('.'));
      if (!middleware)
        middleware = new Middleware(workspace, names.join('.'));
      middleware.setMiddlewarePhase(fileData);
